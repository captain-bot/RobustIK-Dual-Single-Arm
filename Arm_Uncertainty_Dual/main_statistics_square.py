...
Warning: success rates are dependend on the length of transverse side of hole keeping peg cross section fixed
Warning: success strategy: if all the vertices of the peg lies inside the hole then count it as success
...
for _ in range(num_trial):
    ...
    ############################################################################
    ############################ PART 6 ########################################
    ################   Check Collision between Peg and Hole   ##################
    ############################################################################
    if collision_lib.collision_square(P_hole2pegfinal, R_hole2pegfinal):
    ...
    if want_visual:
        animate_assembly_square.visualize_all(T_base2leftgripper, T_base2rightgripper, T_base2peg, T_base2hole, dist,
                                              P_hole2pegfinal, R_hole2pegfinal)

success_rate = (num_trial - failure_count)*100/num_trial
print("Success rate: %2.4f" % success_rate)
